...
- selection rule: three-seed validation high-risk overlap improvement with
  validation reward and Spearman guardrails; test metrics used only for
  reporting
...
- `outputs/best_model/` is the only repo-visible current-best model artifact
  directory and is overwritten on every promotion
- `outputs/generated/` stores ignored local generated runs, candidate datasets,
...
- Preserve V1 under `thesis/versions/v1_submitted_initial_feedback/`; use
  `thesis/Bachelor Thesis Template/` as the overwrite-in-place V2 working draft
  until a later version is explicitly requested.
- Follow scientific language: avoid informal or unquantified claims such as
  "huge", "massive", "spectacular", "really", and similar wording.
- Use IEEE citation style. A citation must not be the subject or object of a
  sentence; the sentence should remain grammatical if the citation is removed.
- Do not add fake, weakly verified, or placeholder references. Every new
  reference must be checked against a real publisher, DOI, arXiv, SSRN, or
  equivalent source before being added.
- Literature work must include a comparison matrix that positions methods by
  dimensions relevant to this thesis, including preselection, investor risk
  profile, adaptivity, RL use, asset coverage, benchmark type, and limitations.
- Paragraphs should each express one coherent idea and should not exceed roughly
  one third of a page.
- Keep the thesis-safe claim: the current promoted model creates distinct
  realized-risk buckets from predicted ranks. Do not claim it proves improved
  final portfolio optimization returns.
- Treat return, Sharpe, Sortino, and drawdown reporting as secondary economic
  diagnostics unless a separate allocation experiment is explicitly added.
- Baseline framing should use a baseline group plus internal ablation unless the
  doctor explicitly requires a single-paper baseline. The closest known paper is
  Orra et al. 2025, but it is not an exact match for the current Egyptian
  mixed-asset, month-level RL risk-ranking setup.
