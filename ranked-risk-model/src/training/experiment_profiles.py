...
class RewardProfile:
    ...
    mse_weight: float
    high_risk_overlap_weight: float = 0.0

    def formula_label(self) -> str:
        label = f"{self.spearman_weight:.2f}*spearman + {self.mse_weight:.2f}*(1-mse)"
        if self.high_risk_overlap_weight:
            label += f" + {self.high_risk_overlap_weight:.2f}*high_risk_top25_overlap"
        return label
...
REWARD_PROFILE_REGISTRY: dict[str, RewardProfile] = {
    ...
    "reward_v5_rank60_mse20_highoverlap20": RewardProfile(
        reward_profile_id="reward_v5_rank60_mse20_highoverlap20",
        description="Tail-aware reward mix: 60% Spearman / 20% (1 - MSE) / 20% highest-risk top-25% overlap.",
        spearman_weight=0.60,
        mse_weight=0.20,
        high_risk_overlap_weight=0.20,
    ),
}
