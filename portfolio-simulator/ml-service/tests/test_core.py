...
from app import mvo as mvo_module
...
from app.data import VALID_SPLITS, available_months, read_daily_market, read_predictions, risk_levels, select_assets
...
from app.precompute import store as store_module
...
def test_health_degrades_when_questionnaire_is_unavailable_without_disabling_store(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    status = store_module.validate_precomputed_store(force=True)
    assert status.available, status.error
    monkeypatch.setattr(simulation_module, "questionnaire_model_available", lambda: False)

    payload = health()

    assert payload["status"] == "degraded"
    assert payload["questionnaireModelAvailable"] is False
    assert payload["optimizerMode"] == "external_model"
    assert payload["optimizerRuntimeAvailable"] is True


def test_precomputed_source_artifacts_exclude_questionnaire_files() -> None:
    artifact_keys = {spec.artifact_key for spec in store_module.source_artifact_specs()}

    assert "questionnaire_contract" not in artifact_keys
...
def test_precomputed_store_validates_schema_and_coverage() -> None:
    with store_module.connect_validated_store() as connection:
        ...
        assert connection.execute("SELECT COUNT(*) FROM decision_snapshots").fetchone()[0] == 111
        assert connection.execute("SELECT COUNT(*) FROM strategy_monthly_returns").fetchone()[0] > 0


def test_precomputed_store_rejects_missing_schema() -> None:
    connection = sqlite3.connect(":memory:")
    connection.row_factory = sqlite3.Row
    with pytest.raises(store_module.PrecomputedStoreValidationError, match="missing table"):
        store_module.validate_store_connection(connection)
    connection.close()


def test_precomputed_store_rejects_generator_version_mismatch(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(store_module, "GENERATOR_VERSION", "old-generator")

    with store_module.connect_store() as connection:
        with pytest.raises(store_module.PrecomputedStoreValidationError, match="generator version"):
            store_module.validate_store_connection(connection)


def test_precomputed_store_rejects_source_hash_mismatch(monkeypatch: pytest.MonkeyPatch) -> None:
    current = store_module.current_source_artifacts()
    first_key = sorted(current)[0]
    current[first_key] = {**current[first_key], "sha256": "0" * 64}
    monkeypatch.setattr(store_module, "current_source_artifacts", lambda: current)

    with store_module.connect_store() as connection:
        with pytest.raises(store_module.PrecomputedStoreValidationError, match="stale"):
            store_module.validate_store_connection(connection)
...
def test_single_mode_exposes_core_benchmark_set() -> None:
    report = run_fast_simulation("2025-03", "medium", duration_months=1, simulator_mode="single")
...
def test_default_mode_exposes_core_benchmark_set() -> None:
    report = run_fast_simulation("2025-03", "medium", duration_months=1, simulator_mode="single")
...
def test_monthly_rebalance_uses_precomputed_decisions_for_each_month() -> None:
    ...
    for index, month in enumerate(["2025-03", "2025-04", "2025-05"]):
        ...
        expected_selected = set(selected["AssetID"].astype(str).tolist())
        actual_selected = {asset["assetId"] for asset in report["rebalanceTimeline"][index]["selectedAssets"]}
...
def test_request_path_does_not_call_live_allocators(monkeypatch: pytest.MonkeyPatch) -> None:
    def fail_optimizer(**_kwargs):
        raise AssertionError("request path must not call live optimizer")

    def fail_mvo(**_kwargs):
        raise AssertionError("request path must not call live MVO")

    monkeypatch.setattr(optimizer_module, "run_weight_optimizer", fail_optimizer)
    monkeypatch.setattr(mvo_module, "run_mvo_allocation", fail_mvo)
    report = simulation_module.run_fast_simulation(
        "2025-03",
        "medium",
        duration_months=3,
...
def test_request_path_uses_cached_store_validation(monkeypatch: pytest.MonkeyPatch) -> None:
    status = store_module.validate_precomputed_store(force=True)
    assert status.available, status.error

    def fail_validation(_connection):
        raise AssertionError("request path must not rerun full store validation")

    monkeypatch.setattr(store_module, "validate_store_connection", fail_validation)
    report = simulation_module.run_fast_simulation(
        "2025-03",
        "medium",
        duration_months=1,
...
def test_fast_endpoint_serializes_multi_month_rebalance() -> None:
...
def test_long_monthly_rebalance_omits_runtime_warning() -> None:
    report = simulation_module.run_fast_simulation(
        "2025-03",
        "medium",
        duration_months=6,
    ...
    assert "Long monthly review windows can take noticeably longer" not in report["thesisSafeSummary"]
    assert_public_historical_summary(report["thesisSafeSummary"])
...
def test_missing_precomputed_store_returns_503(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(store_module, "SIMULATION_STORE_PATH", Path(__file__).resolve().parent / "missing-store.sqlite")
    client = TestClient(app)
    response = client.post("/api/simulations/fast", json={"month": "2025-03", "riskLevel": "medium"})
    assert response.status_code == 503
    assert "Missing precomputed simulation store" in response.json()["detail"]
