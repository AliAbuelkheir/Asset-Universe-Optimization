...
PPO_ROOT = (
    REPO_ROOT / "model-artifacts" / "ranked-risk-model"
    if (REPO_ROOT / "model-artifacts").exists()
    else REPO_ROOT / "ranked-risk-model"
)
...
def read_predictions() -> pd.DataFrame:
    ...
    return _read_predictions(signature[0]).copy()


def _read_monthly_returns(path: str) -> pd.DataFrame:
...
def read_daily_market() -> pd.DataFrame:
    ...
    return _read_daily_market(signature[0]).copy()
...
def read_monthly_returns() -> pd.DataFrame:
    try:
        ...
        raise FileNotFoundError(f"Missing daily market series at {DAILY_MARKET_PATH}") from exc
    return _read_monthly_returns(signature[0]).copy()


def _read_daily_market(path: str) -> pd.DataFrame:
    daily = pd.read_csv(
        path,
        usecols=[
            "Date",
